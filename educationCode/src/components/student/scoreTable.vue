      ...
      <el-table ref="filterTable" :data="score" v-loading="loading">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-tag>{{scope.row.paperId}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="score" label="考试分数" width="200"></el-table-column>
        <el-table-column label="复习效果" width="200">
          <template slot-scope="scope">
            <el-tag :type="scope.row.score>= 60 ? 'success' : 'danger'">{{scope.row.score >= 60 ? "离成功更近一步了" : "再接再厉"}}</el-tag>
          </template>
        </el-table-column>
      </el-table>
    </section>
