    ...
    <div class="wrapper">
      ...
      <ul class="paper" v-loading="loading">
        <li class="item" v-for="(item,index) in pagination.records" :key="index">
          <h4 @click="toExamMsg(item.paperId)">{{item.paperName}}</h4>
          <p class="name">{{item.paperName}}-{{item.type}}</p>
          <div class="info">
            ...
            <i class="iconfont icon-fenshu"></i><span>满分{{item.mark}}分</span>
          </div>
        </li>
      </ul>
...
export default {
  ...
  methods: {
      ...
      this.$axios('/api/exams').then(res => {
        if(res.data.code == 200) {
          ...
          let newPage = allExam.filter(item => {
            return item.paperName.includes(this.key)
          })
    ...
    //跳转到试卷详情页
    toExamMsg(paperId) {
      this.$router.push({path: '/examMsg', query: {paperId: paperId}})
      console.log(paperId)
    }
  }
}
