    ...
    <!--顶部信息栏-->
     <div class="top">
       <ul class="item">
         ...
         <li>{{examData.type}}-{{examData.paperName}}</li>
         <li @click="flag = !flag">
              ...
              <div class="item">
                ...
                <ul>
                  <li v-for="(list, index4) in topic[4]" :key="index4">
                    <a href="javascript:;" @click="short(index4)" :class="{'border': index == index4 && currentType == 4,'bg': shortAnswer[index4][3] == true}"><span :class="{'mark': topic[4][index4].isMark == true}"></span>{{topicCount[0]+topicCount[1]+topicCount[2]+index4+1}}</a>
                  </li>
                </ul>
              </div>
          ...
          <div class="content">
            ...
            <div v-if="currentType == 1">
              <el-radio-group v-model="radio[index]" @change="getChangeLabel" >
                <el-radio :label="1">A、 {{showAnswer.choiceA}}</el-radio>
                <el-radio :label="2">B、 {{showAnswer.choiceB}}</el-radio>
                <el-radio :label="3">C、 {{showAnswer.choiceC}}</el-radio>
                <el-radio :label="4">D、 {{showAnswer.choiceD}}</el-radio>
              </el-radio-group>
            ...
            <div class="fill" v-if="currentType == 2">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li> <el-tag type="success">正确姿势：</el-tag><span class="right">{{topic[2][index].rightAnswer}}</span></li>
                  <li><el-tag>题目解析：</el-tag></li>
            ...
            <div class="judge" v-if="currentType == 3">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li> <el-tag type="success">正确姿势：</el-tag><span class="right">{{topic[3][index].rightAnswer}}</span></li>
                  <li><el-tag>题目解析：</el-tag></li>
            ...
            <div class="short" v-if="currentType == 4">
              <div v-for="(item1,currentIndex1) in part1" :key="currentIndex1">
                <el-input type="textarea" placeholder="请填在此处"
                  v-model="shortAnswer[index][currentIndex1]"
                  rows="8"
                  clearable
                  @blur="shortBG">
                </el-input>
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li> <el-tag type="success">正确姿势：</el-tag><span class="right">{{topic[4][index].rightAnswer}}</span></li>
                  <li><el-tag>题目解析：</el-tag></li>
                  <li>{{topic[2][index].analysis == null ? '暂无解析': topic[2][index].analysis}}</li>
                </ul>
              </div>
...
export default {
  ...
  methods: {
    ...
    getExamData() { //获取当前试卷所有信息
      ...
      this.$axios(`/api/paperInfo/${paperId}`).then(res => {  //通过paperId请求试卷详细信息
        ...
        this.$axios(`/api/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息
          ...
          //
          let len1 = this.topicCount[3]
          let father1 = []
          for(let i = 0; i < len1; i++){
            let children1 = [null,null,null,null]
            father1.push(children1)
          }
          this.shortAnswer = father1
          //
    ...
    shortBG() {
      if(this.showAnswer[this.index][0] != null) {
        this.showAnswer[this.index][3] = true
      }
    },
    fill(index) { //填空题
    ...
    commit() { //答案提交计算分数
      ...
      fillAnswer.forEach((element,index) => { //此处index和 this.index数据不一致，注意
        element.forEach((inner) => {
          if(this.topic[2][index].rightAnswer.includes(inner)) { //判断填空答案是否与数据库一致
            console.log("正确")
      ...
      topic3Answer.forEach((element,index) => {
        ...
        if(right == this.topic[3][index].rightAnswer) { // 当前选项与正确答案对比
      ...
      if(this.time != 0) {
        this.$confirm("考试结束时间未到,是否提前交卷","友情提示",{
          ...
          this.$axios({
            ...
            method: 'post',
            data: {
              paperId: this.examData.paperId, //考试编号
