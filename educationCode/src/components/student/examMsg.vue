...
<template>
  <div id="msg">
    ...
    <div class="wrapper">
      ...
      <ul class="bottom">
        <li>更新于{{examData.createDate}}</li>
        <!--<li>来自 {{examData.institute}}</li>-->
        <li class="btn">{{examData.type}}</li>
        <li class="right"><el-button @click="toAnswer(examData.paperId)">开始答题</el-button></li>
      </ul>
      <ul class="info">
        ...
        <el-collapse-item class="header" name="0">
          <template slot="title" class="stitle" >
            <div class="title">
              ...
              <span class="time">{{examData.mark}}分 / {{examData.totalTime}}分钟</span>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.$axios(`/api/paperInfo/${paperId}`).then(res => {  //通过examCode请求试卷详细信息
        res.data.data.createDate = res.data.data.createDate.substr(0,10)
        this.examData = { ...res.data.data}
        //let paperId = this.examData.paperId
        this.$axios(`/api/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息
          this.topic = {...res.data}
          //console.log("我想看的： "+res.data)
          let keys = Object.keys(this.topic) //对象转数组
          //console.log("我想看的： "+keys)
          keys.forEach(e => {
    ...
    toAnswer(id) {
      this.$router.push({path:"/answer",query:{paperId: id}})
    },
  }
}
</script>
